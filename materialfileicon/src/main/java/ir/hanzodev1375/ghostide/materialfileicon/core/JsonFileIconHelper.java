...
public class JsonFileIconHelper {
  ...
  private static String defaultRootFolder = "default_root_folder";
  ...
  static void apply(JSONObject root) {
    ...
    if (def != null) {
      ...
      defaultRootFolder = def.optString("root_folder", defaultRootFolder);
    }
  ...
  public String getIconName(boolean matchFolderNames) {
    ...
    if (file.isDirectory()) {
      ...
      if (!matchFolderNames) {
        if (dirName.isEmpty()) return defaultRootFolder;
        return defaultFolder;
      }
      String key = dirName.toLowerCase();
      if (!key.isEmpty()) {
