...
public class SuperRipple extends RippleEffect {
  ...
  public static class Effect {
    ...
    private Effect(float cx, float cy, float intensity, ValueAnimator animator) {
      this.cx = cx;
      this.cy = cy;
  ...
  public SuperRipple(View view) {
    ...
    final String code = readRes(view);
  ...
  private static String readRes(View view) {
    StringBuilder builder = new StringBuilder();
    try (BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(
                view.getContext().getResources().openRawResource(R.raw.superripple_effect),
      ...
      throw new IllegalStateException("Failed to read superripple_effect", e);
    }
    return builder.toString();
  }

  private void setupSizeUniforms(boolean force) {
    final float currentDensity = view.getResources().getDisplayMetrics().density;
    if (force
        || width != view.getWidth()
        || height != view.getHeight()
        || Math.abs(density - currentDensity) > 0.01f) {
      shader.setFloatUniform("size", width = view.getWidth(), height = view.getHeight());
      shader.setFloatUniform("density", density = currentDensity);

      final WindowInsets insets = view.getRootWindowInsets();
      final RoundedCorner topLeftCorner =
          insets == null ? null : insets.getRoundedCorner(RoundedCorner.POSITION_TOP_LEFT);
      final RoundedCorner topRightCorner =
          insets == null ? null : insets.getRoundedCorner(RoundedCorner.POSITION_TOP_RIGHT);
      final RoundedCorner bottomLeftCorner =
          insets == null ? null : insets.getRoundedCorner(RoundedCorner.POSITION_BOTTOM_LEFT);
      final RoundedCorner bottomRightCorner =
          insets == null ? null : insets.getRoundedCorner(RoundedCorner.POSITION_BOTTOM_RIGHT);

      shader.setFloatUniform(
          "radius",
          bottomRightCorner == null ? 0 : bottomRightCorner.getRadius(),
          topRightCorner == null ? 0 : topRightCorner.getRadius(),
          bottomLeftCorner == null ? 0 : bottomLeftCorner.getRadius(),
  ...
  public void animate(float cx, float cy, float intensity) {
    ...
    final float speed = 1200 * view.getResources().getDisplayMetrics().density;
    final float max_dist =
        Math.max(
            Math.max(dist(0, 0, cx, cy), dist(view.getWidth(), 0, cx, cy)),
            Math.max(
                dist(0, view.getHeight(), cx, cy),
                dist(view.getWidth(), view.getHeight(), cx, cy)));
  ...
  private void updateProperties() {
    ...
    if (!effects.isEmpty()) {
      ...
      changed = changed || width != view.getWidth() || height != view.getHeight();
      final float currentDensity = view.getResources().getDisplayMetrics().density;
      changed = changed || Math.abs(density - currentDensity) > 0.01f;
      if (changed) {
