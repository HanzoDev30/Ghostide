...
public class AsmIncrementalAnalyzeManager
    extends AsyncIncrementalAnalyzeManager<AsmState, AsmIncrementalAnalyzeManager.HighlightToken> {
  ...
  private final ThreadLocal<AsmTextTokenizer> tokenizerProvider = new ThreadLocal<>();

  protected IdentifierAutoComplete.SyncIdentifiers identifiers =
      new IdentifierAutoComplete.SyncIdentifiers();

  private synchronized AsmTextTokenizer obtainTokenizer() {
    var res = tokenizerProvider.get();
    if (res == null) {
      res = new AsmTextTokenizer("");
  ...
  private static int getType(AsmTokens token) {
    if (token == AsmTokens.LBRACK || token == AsmTokens.RBRACK) return 2;
    if (token == AsmTokens.LPAREN || token == AsmTokens.RPAREN) return 1;
    return 0;
  }

  private static boolean isStart(AsmTokens token) {
    return token == AsmTokens.LBRACK || token == AsmTokens.LPAREN;
  }

  @NonNull
  @Override
  public AsmState getInitialState() {
    return new AsmState();
  }

  @Override
  public boolean stateEquals(@NonNull AsmState state, @NonNull AsmState another) {
    return state.equals(another);
  }

  @Override
  public void onAddState(AsmState state) {
  ...
  @Override
  public void onAbandonState(AsmState state) {
    if (state.identifiers != null) {
      for (String identifier : state.identifiers) {
  ...
  @Override
  public LineTokenizeResult<AsmState, HighlightToken> tokenizeLine(
      CharSequence line, AsmState state, int lineIndex) {
    var tokens = new ArrayList<HighlightToken>();
    int newState = STATE_NORMAL;
    var stateObj = new AsmState();
    ...
    if (tokens.isEmpty()) {
      tokens.add(new HighlightToken(AsmTokens.UNKNOWN, 0));
    }
  ...
  private long tryFillIncompleteComment(CharSequence line, List<HighlightToken> tokens) {
    ...
    if (pre == '*' && cur == '/') {
      if (offset < 1000) {
        detectHighlightUrls(
            line.subSequence(0, offset), 0, AsmTokens.BLOCK_COMMENT_COMPLETE, tokens);
      } else {
        tokens.add(new HighlightToken(AsmTokens.BLOCK_COMMENT_COMPLETE, 0));
      }
      return IntPair.pack(STATE_NORMAL, offset);
    }
    if (offset < 1000) {
      detectHighlightUrls(
          line.subSequence(0, offset), 0, AsmTokens.BLOCK_COMMENT_INCOMPLETE, tokens);
    } else {
      tokens.add(new HighlightToken(AsmTokens.BLOCK_COMMENT_INCOMPLETE, 0));
    }
    return IntPair.pack(STATE_INCOMPLETE_BLOCK_COMMENT, offset);
  }

  private int tokenizeNormal(
      CharSequence text, int offset, List<HighlightToken> tokens, AsmState st) {
    var tokenizer = obtainTokenizer();
    tokenizer.reset(text);
    tokenizer.offset = offset;
    AsmTokens token;
    int state = STATE_NORMAL;
    while ((token = tokenizer.nextToken()) != AsmTokens.EOF) {
      if (tokenizer.getTokenLength() < 1000
          && (token == AsmTokens.STRING_LITERAL
              || token == AsmTokens.CHARACTER_LITERAL
              || token == AsmTokens.BLOCK_COMMENT_COMPLETE
              || token == AsmTokens.BLOCK_COMMENT_INCOMPLETE
              || token == AsmTokens.LINE_COMMENT)) {
        detectHighlightUrls(tokenizer.getTokenText(), tokenizer.offset, token, tokens);
        if (token == AsmTokens.BLOCK_COMMENT_INCOMPLETE) {
      ...
      if (token == AsmTokens.IDENTIFIER
          || token == AsmTokens.INSTRUCTION
          || token == AsmTokens.REGISTER
          || token == AsmTokens.DIRECTIVE) {
        ht.tokenText = tokenizer.getTokenText().toString();
      }
      tokens.add(ht);
      if (token == AsmTokens.LPAREN || token == AsmTokens.LBRACK) {
        st.bracketDepth++;
      } else if (token == AsmTokens.RPAREN || token == AsmTokens.RBRACK) {
  ...
  private void detectHighlightUrls(
      CharSequence tokenText, int offset, AsmTokens token, List<HighlightToken> tokens) {
  ...
  @Override
  public List<Span> generateSpansForLine(LineTokenizeResult<AsmState, HighlightToken> lineResult) {
    ...
    for (int i = 0; i < tokens.size(); i++) {
      ...
      switch (token) {
        ...
        case IDENTIFIER:
          ...
          int j = i + 1;
          AsmTokens next = AsmTokens.UNKNOWN;
          while (j < tokens.size()) {
            AsmTokens n = tokens.get(j).token;
            if (n != AsmTokens.WHITESPACE
                && n != AsmTokens.NEWLINE
                && n != AsmTokens.BLOCK_COMMENT_COMPLETE
                && n != AsmTokens.BLOCK_COMMENT_INCOMPLETE
                && n != AsmTokens.LINE_COMMENT) {
              next = n;
              break;
            }
            j++;
          }
          if (next == AsmTokens.COLON) {
            color = GhostColorScheme.IDENTIFIER_NAME;
          } else if (next == AsmTokens.EQ) {
  ...
  public static class HighlightToken {
    ...
    public HighlightToken(AsmTokens token, int offset) {
      this.token = token;
      this.offset = offset;
    }

    public HighlightToken(AsmTokens token, int offset, String url) {
      this.token = token;
      this.offset = offset;
      this.url = url;
    }
