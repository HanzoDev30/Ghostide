...
public final class BidiLayout {
  ...
  /**
   * Builds or retrieves a cached visual layout for a terminal row. Maps logical terminal grid
   * columns to visual characters and manages RTL/LTR reordering. Uses a fast-path caching strategy
   * to update cursor and selection states in-place, avoiding Bidi calculations and array
   * allocations on frame refreshes.
   */
  public static BidiLayout build(
      TerminalRow rowObject,
      int columns,
      int cursorCol,
    ...
    if (rowObject.mCachedBidiLayout instanceof BidiLayout) {
      ...
      if (cached.visualCells.length == columns) {
        if (isForRendering) {
          ...
          int caretCol =
              cursorVisible
                  ? caretVisualColumn(cached.visualToLogical, cached.visualCells, cursorCol)
                  : -1;
    ...
    for (int column = 0; column < columns; ) {
      ...
      while (currentCharIndex < charsUsedInLine) {
        ...
        int nextCp =
            nextIsHigh ? Character.toCodePoint(nextChar, line[currentCharIndex + 1]) : nextChar;
   ...
   * precedes the logical cursor column. This keeps the caret on the correct side of right-to-left
   * text: after typing a Persian word the caret appears at the left edge of the word instead of
   * trailing off to the right, and it stays adjacent to the characters being typed.
   */
  private static int caretVisualColumn(
      int[] visualToLogical, LogicalCell[] visualCells, int cursorCol) {
